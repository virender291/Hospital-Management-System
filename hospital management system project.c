...
struct patient//list of global variable (its available anywhere in code)
{ 
	int age;
	char Gender;
	char First_Name[20];
...
void WelcomeScreen(void) // for welcomescreen function
{
	...
	getch();//holds screen for some second
	
	
}
void title(void)//for title function
{
	printf("\n\n\t\t----------------------------------------------------------------------------------");
	printf("\n\t\t\t\t       International Hospital         ");
	printf("\n\t\t----------------------------------------------------------------------------------");
}
void mainmenu(void)//for mainmenu function
{
	system("cls");
	int choose;
	title();
...
switch(choose)
{
    ...
    case 2:
    	...
    	break;
	case 3:
	search_record();//View_rec function is call
    	break;
	case 4:
		edit_record();//Edit_rec function is call
		break;
	case 5:
		delete_record();//Dlt_rec function is call
		break;
	case 6:
		ex_it();
    	break;
...
char Username[15];
char Password[15];
char original_Username[25]="virenderchawla";
char original_Password[15]="abcde";
...
void add_record(void)
{
   ...
   //############################ First Name ###############################
	...
	else
	{
		...
		if(!valid)
		{
			printf("\n\t\t First name contain Invalid character :(  please Enter last name again :)");
			goto A;
		}
	}
...
//################################ Problem ######################################
    ...
    fprintf(fptr," %s %s %c %i %s %s %s %s\n", p.First_Name, p.Last_Name, p.Gender, p.age, p.Address,
	 p.Contact_no, p.Problem, p.Doctor);
    printf("\n\n\t\t\t.... Information Record Successful ...");
    fclose(fptr);//fptr file is closed
    ab:
    getch();
...
void patient_list()
{
	...
	gotoxy(1,15);
		...
		printf("=================================================================================================================");
		row=17;
		while(fscanf(fptr,"%s %s %c %i %s %s %s %s\n", p.First_Name, p.Last_Name, 
					&p.Gender, &p.age, p.Address, p.Contact_no, p.Problem, p.Doctor)!=EOF)
		{
			gotoxy(1,row);
...
void search_record(void)
{
	...
	while(fscanf(fptr,"%s %s %c %i %s %s %s %s\n", p.First_Name, 
	p.Last_Name, &p.Gender, &p.age, p.Address, p.Contact_no, p.Problem, p.Doctor)!=EOF)
	{
	   ...
	   if(fptr==NULL)
	   {
		 ...
		 p.Last_Name, &p.Gender, &p.age, p.Address, p.Contact_no, p.Problem, p.Doctor)!=EOF)
		{
			if(strcmp(p.First_Name, name)==0)
			{
				...
				p.Last_Name,p.Gender, p.age,p.Address,p.Contact_no,p.Problem,p.Doctor);
...
void delete_record()
{
	...
	while (fscanf(fptr,"%s %s %c %i %s %s %s %s", p.First_Name, p.Last_Name, &p.Gender,
			 &p.age, p.Address, p.Contact_no, p.Problem, p.Doctor)!=EOF)
	{
		if (strcmp(p.First_Name,name)!=0)
		fprintf(ft,"%s %s %c %i %s %s %s %s\n", p.First_Name,
		 p.Last_Name, p.Gender, p.age, p.Address, p.Contact_no, p.Problem, p.Doctor);
